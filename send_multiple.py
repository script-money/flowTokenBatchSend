...
async def send_multiple(addresses_arg: cadence.Dictionary):
    async with flow_client(
        ...
        tx = (
            Tx(
                code=open(
                    Path(__file__).parent.joinpath("./transfer_many_accounts.cdc")
                ).read(),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Send F to multiple accounts")
    parser.add_argument("amount", type=float, help="flowToken amount (.8f)")
    args = parser.parse_args()
    send_amount = vars(args)["amount"]
    asyncio.run(main("flow_test.csv", send_amount))
